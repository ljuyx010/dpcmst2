jQuery(function() {
	...
	uploader.onUploadSuccess=function(file ,response){
		var div=$('<div class="input-group " id="imageDelete'+response.id+'" style="margin-top:.5em;margin-right:2em; float:left;"><input type="hidden" name="imagepath[]" class="imagepath" value="'+response.id+'"/><img src="'+response.imagepath+'"; class="img-responsive img-thumbnail" width="150"><a class="close" style="position:absolute; top: 0px; right: -14px;" title="删除这张图片" onclick="deleteImage('+response.id+')">×</a></div>');
		$('#formimageshow').after(div);
		var imagepath=$('input[class=imagepath]');
		if(imagepath.length>=10){

			$('#filePicker2').attr("disabled","disabled");//禁用上传按钮
			$("#upload").fadeIn();//关闭上传
			$("#upload").fadeOut('slow');
		}else{

		}
		//$('#'+file.id +' .bjy-filename').val(response.imagepath)
	}
